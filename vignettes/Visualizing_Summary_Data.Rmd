...
In this vignette we will demo how to visualize data only available in summary format e.g. data coming from a published paper table:
...
remdesivirdata <- read.csv("./remdesivirfig3.csv")
...
# from R lauch the app with the data 
...
* Go to **Graph Options** tab and select the **Facets Options** subtab. We will be using advanced Faceting (graph splitting options):
    + Set **Facet Scales** to 'free_y' (this allows only subgroupvalues specific to the subgroup to appear on each graph panel)
    + Set **Facet Spaces** to 'free_y' (this allows for each panel to occupy space proportional to the number of subgroupvalues)
...
![reordering of subgroups](./sumdata4.png){width=50%}

While you can add another variable and manually drag and drop we will demo next another possibility to reorder yvalues using a statisitc of another variable e.g. median:
...
![reordering of subgroupsvalue](./sumdata5.png){width=50%}
