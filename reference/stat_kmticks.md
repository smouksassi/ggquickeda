...
This stat is for computing the tick marks for a Kaplan-Meier survival
estimate for right-censored data. The tick marks will appear at each
censoring time which is also not a death time, which is the default for
[plot.survfit](https://rdrr.io/pkg/survival/man/plot.survfit.html) It
requires the aesthetic mapping `x` for the observation times and
`status` which indicates the event status, normally 0=alive, 1=dead.
